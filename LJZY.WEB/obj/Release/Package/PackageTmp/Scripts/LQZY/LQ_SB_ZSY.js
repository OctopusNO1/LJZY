...
$(function () {
    ...
    //表格数据源
    $('#dg').datagrid({
        url: '../../Controllers/SBGLController.ashx?action=SB_ZSYDataGrid',
        loadMsg: '数据加载中......',
...
function insert_one() {
    ...
    //打开窗体
    $("#insert_dlg").dialog("open").dialog("setTitle", "钻时仪设备信息导入");
...
function excelImport() {
    ...
    $('#jurSubsystemMenu').form('submit', {
        url: "../../Controllers/SBGLController.ashx?action=PreviewExcel_ZSY",
        onSubmit: function () {
            return true;
        },
...
function PreviewImport() {
    ...
    $('#girdPreview').datagrid({
        ...
        columns: [[     
            ...
            { field: 'CCBH', title: '出厂编号', align: 'center', width: 100 },
            { field: 'TCRQ_GD', title: '投产日期', align: 'center', width: 100 },
            { field: 'ZJNX', title: '设备现编号', align: 'center', width: 100 },
            { field: 'RLFL', title: '设备自编号', align: 'center', width: 100 },
            { field: 'SBZK', title: '设备状况', align: 'center', width: 100 },
            { field: 'DW', title: '所属单位', align: 'center', width: 100 },
...
function openUrl() {
    if (rydata.length > 0) {
        $.ajax({
            type: "POST",
            url: '../../Controllers/SBGLController.ashx?action=ImportExcel_ZSY',
...
function exportExcel() {
    ...
    $('#ExportIframe').attr("src", "../../../Page/DownLoad/DownLoad.aspx?Source=OutExportExcel_SBZSY&str=" + escape(str));//escape()特殊字符处理
}
