...
$(function () {
    ...
    //表格数据源
    $('#dg').datagrid({
        url: '../../Controllers/SBGLController.ashx?action=SB_DHFXDataGrid',
        loadMsg: '数据加载中......',
...
function insert_one() {
    ...
    //打开窗体
    $("#insert_dlg").dialog("open").dialog("setTitle", "地化分析设备信息导入");
...
function excelImport() {
    ...
    $('#jurSubsystemMenu').form('submit', {
        url: "../../Controllers/SBGLController.ashx?action=PreviewExcel_DHFX",
        onSubmit: function () {
            return true;
        },
...
function openUrl() {
    if (rydata.length > 0) {
        $.ajax({
            type: "POST",
            url: '../../Controllers/SBGLController.ashx?action=ImportExcel_DHFX',
...
function exportExcel() {
    $('#ExportIframe').attr("src", "../../../Page/DownLoad/DownLoad.aspx?Source=OutExportExcel_SBDHFX&str=" + escape(str));
}
