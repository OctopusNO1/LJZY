...
$(function () {
    ...
    //表格数据源
    $('#dg').datagrid({
        url: '../../Controllers/DJCHYAQHSEController.ashx?action=DJCHYAQSHE_DataGrid&type=1',
        loadMsg: '数据加载中......',
...
function insert() {
    ...
    //打开窗体
    $("#insert_dlg").dialog("open").dialog("setTitle", "单井应急预案导入");
...
function openUrl() {
    if (rydata.length > 0) {
        $.ajax({
            type: "POST",
            url: '../../Controllers/DJCHYAQHSEController.ashx?action=ImportExcel_DJCHYAHSE&type=1',
